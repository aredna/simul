...
| Useful search and text fields can contain public text, but form controls may also contain secrets. | Isolated HTML reads only native text, search, email, URL, telephone, and textarea controls. It uses a nonempty current value or otherwise the placeholder. Native selects contribute only their visible option labels and current selection; raw option values, names, and form semantics never cross the boundary. Passwords, password-autocomplete fields, unsupported controls, contenteditable regions, and ARIA textbox fallbacks stay blank. Control text is typed metadata rather than a copied `value` attribute and is never logged or saved. |
...
faithful translated-text layout. **Isolated HTML** is the replica engine; the
earlier experimental rrweb engine was removed in 0.4.0. A
...
without a supported language hint remain unchanged. Pixels that overlap a
password or other private control are never captured; overlap with an ordinary
public text control does not block capture, because that text is already shown
in the replica. OCR overlays are inert and
