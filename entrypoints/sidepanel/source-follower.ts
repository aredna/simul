...
export interface FollowerBrowser {
  getTab(tabId: number): Promise<PageTabLike>;
...
export interface SourceFollowerEnvironment {
  ...
  readonly onSourceNavigationStarted: () => void;
...
export class SourceFollower {
  ...
  async acceptAuthorizedTab(request: AuthorizedTabRequest): Promise<void> {
    ...
    if (request.launchStamp) {
      if (!isNewerCompanionLaunchStamp(state.latestToolbarLaunchStamp, request.launchStamp)) {
        return;
      }
  ...
  async followActivatedSourceTab(
    ...
    try {
      ...
      const identity = identityFromTab(knownTab ?? await browser.getTab(tabId), undefined, true);
      if (!currency.isCurrent(request) || state.preferences.popoutTabMode !== 'active') return;
  ...
  handleTabUpdated(tabId: number, changeInfo: TabChangeInfo, tab: UpdatedTab): void {
    ...
    if (changeInfo.status === 'loading' || typeof changeInfo.url === 'string') {
      this.environment.onSourceNavigationStarted();
      state.followedPageIdentity = nextIdentity;
      this.#clearNavigationTimer();
  ...
  handleTabRemoved(tabId: number): void {
    if (this.#state.followedPageIdentity?.tabId === tabId) {
  ...
  async #followMovedLockedSourceTab(
    ...
    const { currency } = this.environment;
    try {
      const identity = identityFromTab(await this.environment.browser.getTab(tabId), undefined, false);
      if (
  ...
  async #followFocusedBrowserWindow(windowId: number, request: CurrencyToken): Promise<void> {
    const state = this.#state;
    const { currency, browser } = this.environment;
    try {
      const sourceWindow = await browser.getWindow(windowId);
  ...
  #scheduleNavigationRefresh(identity: CapturedPageIdentity): void {
    ...
    this.#navigationTimer = setTimeout(() => {
      ...
      if (followed?.tabId !== identity.tabId || followed.windowId !== identity.windowId) return;
      this.environment.queueCapture({ identity, reason: 'navigation' });
