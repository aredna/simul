...
- **Image translation on by default (D47).** Owner direction. Pixel OCR still
...
- **Caption band for label-based image translations (D48).** An
  ...
  everything.
- **Version is 0.5.0, identity `0.5.0 beta v.20260922.9`.** `package.json`,
  ...
  shows `0.5.0`; Simul's settings show `Build 0.5.0 beta v.20260922.9`. What is
...
`Build 0.5.0 beta v.20260922.9`; anything else means the old folder is still
loaded.
...
`0.5.0 beta v.20260922.9`), so what remains mirrors the 0.4.0 publish (D32): a
...
If anything else ships before the merge, bump the suffix (`.10` after the
D50 build) and re-sync first.
...
git tag -a v0.5.0 -m "Simul 0.5.0 beta (v.20260922.9)"
...
gh release create v0.5.0 simul-0.5.0-chrome-unpacked.zip \
  --prerelease --title "Simul 0.5.0 beta (v.20260922.9)" \
...
Then log it as D51 (merge SHA, tag, release URL, gate numbers) and update the
...
settings show `Build 0.5.0 beta v.20260922.9`. This is an unpacked beta, so
...
- **Image text on by default, and it translates the page too.** Image
  ...
  translated together; no separate Translate click is needed for the page.
...
- **Carousels stay readable.** A slide container that previous/next buttons
  ...
  state, or a tab relation, can withhold what it controls. And a slide change
  that rewrites classes and transforms in one tick no longer turns the
  slides, the wrapper or the pagination into blank placeholders: that
  credential rule now applies to regions holding value-bearing controls, not
