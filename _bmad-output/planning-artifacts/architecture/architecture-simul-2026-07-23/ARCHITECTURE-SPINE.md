...
- **Rule:** Walk the unified method order lazily. Read semantic evidence only when its position is reached; decisive semantic evidence ends the walk, while provisional evidence advances only far enough to obtain the minimum eligible later OCR evidence needed for a decision. Acquire pixels only on reaching that enabled OCR method; never sweep every provider merely to rank candidates. Group contiguous OCR methods while preserving their exact order, hints, independent-family corroboration, confidence filtering, retries, and cache behavior. No provider-specific empty result may skip a later method, and a group-level host failure returns to the planner so later semantic or OCR groups may run. If pixel access is absent, OCR is ineligible, every provider is empty or unavailable, or the bounded transient retry is exhausted, use the best current semantic evidence. `skipSmallImages` gates small OCR captures only; positive-area accessibility text remains eligible, while zero-area and tracking-pixel exclusions remain. Optional pixel-capture permission is required only when an enabled route reaches pixel OCR.

### AD-9 — Image evidence has provenance-neutral projection geometry

- **Binds:** IMG-1
- **Prevents:** Forcing semantic labels through OCR box/bitmap contracts, translating a losing candidate, and overlaying stale or unsafe markup.
- **Rule:** Select source evidence before translation or projection. The bounded deterministic ranker may use exact semantic/OCR agreement, existing accepted OCR confidence and independent-family corroboration, role-free text shape, exact-document repetition, and the saved order; it cannot weaken the OCR quality gate or treat the two Tesseract bindings as independent. Only the selected evidence may translate, project, or vote for Auto language. Normalize that evidence into image-relative regions with explicit `methodId`, `evidenceKind`, and placement. OCR boxes normalize through captured crop geometry. Accessibility text projects as one clipped, centered, inset translucent label over the image rather than whitening the bitmap. Rendering assigns translated strings through `textContent`, is pointer-inert, and keys freshness to document, node, content revision, method configuration, language pair, evidence identity, and ranking-policy version. Diagnostics carry only the selected evidence kind and allowlisted decision reason plus existing method/status/counts, never source text, translated text, identifiers, hashes, lengths, or per-label frequencies.
...
- **Rule:** Keep scheduling descriptors free of URL, text, pixels, and hashes. Extend the exact-document image-source protocol with a strict evidence-read request and `ready | none | blocked | stale` response. A bounded memory-only exact-document index may compare normalized semantic evidence across current images; duplicate-group formation or dissolution cancels affected active peers and requeues only exact-current images whose ranking state changed. Observe `alt`, `aria-label`, `aria-hidden`, and `role` mutations as content revisions so in-flight work is cancelled, the prior candidate and language vote are removed, and affected evidence is re-evaluated. Apply the hard-secret floor during discovery and the selected `controlImages` admission gate at evidence/pixel read time. Both rrweb and isolated-HTML bridges use this one source-image session contract.
...
- **Rule:** Persist settings only. Source values, accessibility text, OCR transcripts, translations, pixels, replica DOM, candidate evidence, normalized-label repetition state, and ranking state remain bounded in memory except the existing short-lived transient pixel handoff, which is removed in `finally`, expires, and is cleared on reset. Candidate and repetition state is invalidated on source reconnect, label mutation/removal, method or read-policy changes, language-pair/reset transitions, release, and disposal. Logs and diagnostics are content-free; no ranking example or feedback is collected or exported. No remote executable code, analytics payload, new ranking-model weight or runtime, or new network service is introduced.
