...
export function assertSourceTabIsCurrent(
  tab: PageTabLike | undefined,
...
export function readAuthorizedTabMessage(
  message: unknown,
): AuthorizedTabRequest | undefined {
  if (
...
/**
 * The document a navigation refresh belongs to: tab, window and URL without
 * credentials or hash. A query change is a different document; a hash change
 * is not.
 */
export function navigationPageIdentityKey(identity: CapturedPageIdentity): string {
...
/** The tab a navigation refresh belongs to, independent of its document. */
export function navigationPageScopeKey(identity: CapturedPageIdentity): string {
...
export function withPageTimeout<T>(
  operation: Promise<T>,
  milliseconds: number,
  message = PAGE_TIMEOUT_GUIDANCE,
): Promise<T> {
  return new Promise<T>((resolve, reject) => {
    const timeout = setTimeout(
      () => reject(new PageAccessError(message)),
      milliseconds,
    );
    operation.then(
      (value) => {
