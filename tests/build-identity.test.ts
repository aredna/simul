...
describe('extension build identity', () => {
  it('renders and logs the trimmed runtime manifest version name', () => {
    const identity = createExtensionBuildIdentity({
      version: '0.5.1',
      version_name: ' 0.5.1 beta v.20260924.2 ',
    });
    ...
    expect(identity).toEqual({
      version: '0.5.1',
      label: 'Build 0.5.1 beta v.20260924.2',
      companionReadyMessage:
        '[Simul] Companion ready. Build 0.5.1 beta v.20260924.2.',
      backgroundReadyMessage:
        '[Simul] Background service worker ready. Build 0.5.1 beta v.20260924.2.',
    });
    expect(target.textContent).toBe('Build 0.5.1 beta v.20260924.2');
  });
