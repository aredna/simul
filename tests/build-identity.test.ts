...
describe('extension build identity', () => {
  it('renders and logs the trimmed runtime manifest version name', () => {
    const identity = createExtensionBuildIdentity({
      version: '0.5.2',
      version_name: ' 0.5.2 beta v.20260925.7 ',
    });
    ...
    expect(identity).toEqual({
      version: '0.5.2',
      label: 'Build 0.5.2 beta v.20260925.7',
      companionReadyMessage:
        '[Simul] Companion ready. Build 0.5.2 beta v.20260925.7.',
      backgroundReadyMessage:
        '[Simul] Background service worker ready. Build 0.5.2 beta v.20260925.7.',
    });
    expect(target.textContent).toBe('Build 0.5.2 beta v.20260925.7');
  });
