...
describe('extension build identity', () => {
  it('renders and logs the trimmed runtime manifest version name', () => {
    const identity = createExtensionBuildIdentity({
      version: '0.5.0',
      version_name: ' 0.5.0 beta v.20260922.32 ',
    });
    ...
    expect(identity).toEqual({
      version: '0.5.0',
      label: 'Build 0.5.0 beta v.20260922.32',
      companionReadyMessage:
        '[Simul] Companion ready. Build 0.5.0 beta v.20260922.32.',
      backgroundReadyMessage:
        '[Simul] Background service worker ready. Build 0.5.0 beta v.20260922.32.',
    });
    expect(target.textContent).toBe('Build 0.5.0 beta v.20260922.32');
  });
