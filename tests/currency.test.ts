...
describe('Currency', () => {
  ...
  it('keeps scopes independent so an unrelated check cannot cancel a follow', () => {
    ...
    currency.begin('availability');
    currency.supersede('image-access', 'language-resolution');
  ...
  it('cannot accept a token against a different scope', () => {
    ...
    // A token minted for one scope carries that scope with it, so a guard
    // written against another scope compares the right counter anyway.
  ...
  it('supersedes every page-scoped token at once but leaves device scopes alone', () => {
    ...
    expect(PAGE_CURRENCY_SCOPES).toContain('language-resolution');
  });
